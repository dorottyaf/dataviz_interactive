...
function styleFeature(feature) {
    ...
    const status = countryStatusMap[countryName]?.[`${year}_A1`];
    return {
        ...
        data.forEach(row => {
            ...
            if (country) {
                ...
                for (let year = 2013; year <= 2024; year++) {
                    countryStatusMap[country][`${year}_A1`] = row[`${year}_A1`];
                }
            }
});
    },
...
fetch('https://raw.githubusercontent.com/datasets/geo-countries/master/data/countries.geojson')
    ...
    .then(geojson => {
        ...
        timeDimensionElections = L.timeDimension.layer.choropleth(geoJsonLayer, {});
        // make it globally accessible
        window.timeDimensionElections  = timeDimensionElections;

        // make event handler
        const event = new Event('timeDimensionElectionsReady');
