...
function styleFeature(feature) {
    ...
    const status = countryStatusMap[countryName]?.[`${year}_D4`];
    return {
        ...
        data.forEach(row => {
            ...
            if (country) {
                ...
                for (let year = 2013; year <= 2024; year++) {
                    countryStatusMap[country][`${year}_D4`] = row[`${year}_D4`];
                }
            }
});
    },
...
fetch('https://raw.githubusercontent.com/datasets/geo-countries/master/data/countries.geojson')
    ...
    .then(geojson => {
        ...
        timeDimensionSpeech = L.timeDimension.layer.choropleth(geoJsonLayer, {});
        // make it globally accessible
        window.timeDimensionSpeech  = timeDimensionSpeech;

        // make event handler
        const event = new Event('timeDimensionSpeechReady');
